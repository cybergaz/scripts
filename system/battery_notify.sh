...
while [ true ]; do
  ...
  if [[ $(cat /sys/class/power_supply/BAT*/status) != "Discharging" ]]; then # -- charging state
    ...
    if (($capc == 100)); then
      if ((full_flag != 1)); then
        notify-send -i battery-charged "Battery" "<i>replenished completely</i>"
        full_flag=1
      fi
    fi
    SLEEP_TIME=30
    ...
    if (($capc >= 60)); then
      ...
      if (($capc <= 10)); then
        SLEEP_TIME=20
        if ((low_flag != 1)); then
          notify-send -i battery-caution "Battery" "<i>${capc}% remaining</i>"
          low_flag=1
        fi
      ...
      if (($capc <= 5)); then
        SLEEP_TIME=15
        if ((crit_flag != 1)); then
          notify-send -i battery-caution "Battery" "<i>find the charge ASAP</i>" -u critical -t 6000
          crit_flag=1
        fi
      fi
      if (($capc <= 3)); then
        ...
        if ((vcrit_flag != 1)); then
          notify-send -i battery-empty "Battery" "<i>shutting down in 1 minute</i>" -u critical -t 8000
          shutdown --no-wall
